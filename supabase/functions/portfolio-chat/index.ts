...
serve(async (req) => {
  ...
  try {
    ...
    console.log("🔍 Checking for contact form submission...");
    console.log("📝 Contact form data received:", contactFormData);
    ...
    if (
      ...
      console.log(
        "✅ All contact form fields present, proceeding with submission..."
      );

      // Validate all fields
      console.log("🔍 Validating contact form fields...");
      ...
      if (!checkContactRateLimit(clientIp)) {
        return new Response(
          JSON.stringify({
            error:
              "You've reached the maximum number of contact submissions. Please try again in an hour.",
            retryAfter: 3600,
          }),
          {
            status: 429,
      ...
      if (isDuplicate) {
        return new Response(
          JSON.stringify({
            error:
              "This message was already submitted recently. Please wait a few minutes before submitting again.",
          }),
          {
            status: 429,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      try {
        ...
        console.log("✅ Contact submission process completed successfully");
        ...
        if (limit && now < limit.resetTime) {
          ...
          contactRateLimiter.set(clientIp, {
            count: 1,
            resetTime: now + CONTACT_RATE_WINDOW,
          });
        }
    ...
    }'s professional portfolio website.
...
SPECIALIZATIONS:
${knowledgeBase.personal.specializations
  .map((s: string) => `- ${s}`)
  .join("\n")}
