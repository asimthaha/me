...
export const PortfolioChatbot = () => {
  ...
  const sendMessageWithRetry = async (attempt: number = 0): Promise<void> => {
    ...
    if (attempt === 0) {
      ...
      const cancelWords = ['cancel', 'stop', 'nevermind', 'never mind', 'no thanks'];
      ...
      if (isCancellation && isInContactFlow) {
        ...
        setMessages(prev => [...prev, userMessage, {
          role: 'assistant',
          content: "No problem! The contact form has been cancelled. Feel free to ask me anything else about the portfolio.",
          id: Date.now().toString(),
        }]);
        setInput("");
        return;
      ...
      if (!isInContactFlow) {
        const contactIntents = ['contact', 'reach out', 'get in touch', 'send message', 'email', 'talk to'];
    ...
    const confirmationWords = ['yes', 'confirm', 'send', 'submit', 'correct', 'ok'];
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        if (jsonResponse.success) {
          ...
          setMessages((prev) => [
            ...
            {
              role: "assistant",
              content: jsonResponse.message || "Thank you! Your message has been sent successfully. I'll get back to you soon! 🎉",
              id: Date.now().toString(),
            },
          ]);
          setContactFormData({});
            ...
            {isInContactFlow && (
              <div className="bg-accent/10 border border-accent/20 rounded-lg p-3 text-sm">
                <div className="flex items-center justify-between mb-2">
                  <p className="font-semibold text-accent">📝 Contact Form Progress</p>
                  {showCancelButton && (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => {
                        ...
                        setMessages(prev => [...prev, {
                          role: 'assistant',
                          content: "Contact form cancelled. How else can I help you?",
                          id: Date.now().toString(),
                        }]);
                      }}
