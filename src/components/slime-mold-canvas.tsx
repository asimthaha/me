...
const SlimeMoldCanvas = ({ isAnimating, className = '' }: SlimeMoldCanvasProps) => {
  ...
  useEffect(() => {
    ...
    // Initialize agents
    const numAgents = Math.min(300, Math.floor((canvas.width * canvas.height) / 1000));
    agentsRef.current = Array.from({ length: numAgents }, () => 
  ...
  useEffect(() => {
    ...
    const animate = (currentTime: number) => {
      if (currentTime - lastTime >= frameInterval) {
        ...
        ctx.fillStyle = isDarkMode ? 'rgba(0, 0, 0, 0.05)' : 'rgba(255, 255, 255, 0.03)';
        ...
        for (let i = 0; i < trailMap.data.length; i += 4) {
          ...
          if (isDarkMode) {
            ...
            imageData.data[i] = Math.floor(intensity * 100);     // R
            imageData.data[i + 1] = Math.floor(intensity * 150); // G
            imageData.data[i + 2] = Math.floor(intensity * 120); // B
            imageData.data[i + 3] = Math.floor(intensity * 40);  // A
          } else {
            // Light mode: subtle gray trails
            imageData.data[i] = Math.floor(intensity * 120);     // R
            imageData.data[i + 1] = Math.floor(intensity * 140); // G
            imageData.data[i + 2] = Math.floor(intensity * 160); // B
            imageData.data[i + 3] = Math.floor(intensity * 25);  // A
          }
        }
